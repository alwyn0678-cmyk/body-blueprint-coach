...
// Safe number guard — returns fallback if value is NaN, null, undefined, or non-finite
...
export const Progress: React.FC = () => {
  ...
  // Guard: if logs object is empty or user is missing, allTrendData will be []
  ...
  const nutritionChart = useMemo(() => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      // Guard NaN in macro values
      const cal = Math.max(0, isNaN(macros.calories) ? 0 : Math.round(macros.calories));
      const pro = Math.max(0, isNaN(macros.protein)  ? 0 : Math.round(macros.protein));
      if (log || timeframe === '1W') {
        results.push({
          name: timeframe === '1W'
          ...
          cal,
          pro,
          target: safeNum(user?.targets.calories, 0),
        });
      }
    }
    return results;
  ...
  // Goal progress — guard divide by zero
  const weightToGoal = (user?.goalWeight != null && !isNaN(user.goalWeight))
    ? Math.abs(safeNum(user.weight) - user.goalWeight)
    : null;

  const weightProgress = useMemo(() => {
    if (!user?.goalWeight || user.goalWeight == null) return null;
    const currentW = safeNum(user.weight);
    const goalW    = safeNum(user.goalWeight);
    const trendAdj = parseFloat(trendDelta || '0');

    // Denominator = initial distance to goal — guard against < 0.1
    let denominator = Math.abs(currentW - goalW);
    if (denominator < 0.1) denominator = 1; // user has effectively reached goal

    const numerator = Math.abs(currentW - goalW);
    const pct = Math.max(0, Math.min(100, (1 - numerator / denominator) * 100));
      ...
      <div className="flex-col gap-3" style={{ padding: '1rem' }}>
            ...
            {[
              {
                ...
                value: `${isNaN(weeklyStats.calorieAdherence) ? 0 : Math.round(weeklyStats.calorieAdherence)}%`,
                label: 'Cal Adherence',
                color: calAdherenceColor,
                pct: isNaN(weeklyStats.calorieAdherence) ? 0 : Math.min(100, weeklyStats.calorieAdherence),
              },
              {
                icon: <Target size={14} />,
                value: `${isNaN(weeklyStats.proteinAdherence) ? 0 : Math.round(weeklyStats.proteinAdherence)}%`,
                label: 'Protein Hit',
                color: proAdherenceColor,
                pct: isNaN(weeklyStats.proteinAdherence) ? 0 : Math.min(100, weeklyStats.proteinAdherence),
              },
              {
                icon: <Flame size={14} />,
                ...
                pct: Math.min(100, (weeklyStats.daysLogged / 7) * 100),
              },
            ].map(card => (
              <div key={card.label} style={{
                ...
                <div style={{
                  ...
                  height: `${isNaN(card.pct) ? 0 : card.pct}%`,
                  maxHeight: '100%',
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem', marginBottom: '0.875rem' }}>
              <div className="flex-col align-center" style={{ gap: '2px' }}>
                <span style={{ fontSize: '1.25rem', fontWeight: 800, fontVariantNumeric: 'tabular-nums', color: 'var(--text-primary)' }}>
                  {(user.weight != null && !isNaN(user.weight)) ? user.weight : '—'}
                </span>
              ...
              <div className="flex-col align-center" style={{ gap: '2px' }}>
                <span style={{
                  ...
                  fontWeight: 800,
                  fontVariantNumeric: 'tabular-nums',
                  color: weightToGoal != null && weightToGoal < 1 ? 'var(--accent-green)' : 'var(--accent-blue)',
                }}>
                  {weightToGoal != null && !isNaN(weightToGoal) ? weightToGoal.toFixed(1) : '—'}
                </span>
        ...
        <div style={{
          ...
          <div style={{ padding: '1.125rem 1.125rem 1rem' }}>
            ...
            {/* Guard: only render chart if there is data */}
            {chartData.length > 0 ? (
              ...
              <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.25)', fontWeight: 600 }}>No weight data yet — log your weight to see trends</span>
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="flex-row gap-3" style={{ marginTop: '4px' }}>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>
                      Avg: <strong style={{ color: 'var(--text-primary)' }}>
                        {isNaN(weeklyStats.avgCalories) ? '—' : weeklyStats.avgCalories} kcal
                      </strong>
                    </span>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>
                      Protein: <strong style={{ color: 'var(--color-protein)' }}>
                        {isNaN(weeklyStats.avgProtein) ? '—' : weeklyStats.avgProtein}g
                      </strong>
                    </span>
                  </div>
                )}
                ...
                <BarChart data={nutritionChart} margin={{ top: 0, right: 8, left: -22, bottom: 0 }}>
                  ...
                  <Bar dataKey="cal" radius={[3, 3, 0, 0]} barSize={timeframe === '3M' ? 4 : 10} name="Calories">
                    {nutritionChart.map((entry, i) => (
                      <Cell
                        key={i}
                        fill={entry.cal > entry.target && entry.target > 0
                          ? 'var(--accent-red)'
          ...
          <div style={{
            ...
            <div style={{ padding: '0.5rem 0' }}>
              {[
                ...
                const safeTarget = Math.max(1, row.target); // avoid divide-by-zero
