...
export const MacroSummaryCard: React.FC<MacroSummaryCardProps> = ({
  calories, protein, carbs, fats, targets, onLogFood,
}) => {
  const calRem  = Math.max(0, targets.calories - Math.round(calories));
  const calPct  = (calories / targets.calories) * 100;
  const isOver  = calories > targets.calories;
  ...
  return (
    <div style={{
        ...
        >
          <div className="flex-col align-center justify-center" style={{ gap: '2px' }}>
            ...
            <span style={{
              ...
              {isOver ? Math.round(calories - targets.calories) : calRem}
            </span>
            <span style={{
              fontSize: '0.5625rem', fontWeight: 700,
              color: isOver ? 'var(--accent-red)' : 'var(--color-calories)',
            ...
            <span style={{
              ...
              {Math.round(calories)} / {targets.calories}
            </span>
          </div>
        </ProgressRing>
      </div>
      ...
      <div style={{
        ...
        {[
          { label: 'Protein', abbr: 'P', value: Math.round(protein),  target: targets.protein,  color: 'var(--color-protein)' },
          { label: 'Carbs',   abbr: 'C', value: Math.round(carbs),    target: targets.carbs,    color: 'var(--color-carbs)'   },
          { label: 'Fats',    abbr: 'F', value: Math.round(fats),     target: targets.fats,     color: 'var(--color-fats)'    },
        ].map(m => {
...
export const DailyHabitsCard: React.FC<DailyHabitsCardProps> = ({
  ...
  const stepsPct = Math.min(100, (steps / targetSteps) * 100);
        ...
        >
          ...
          <div>
            <div style={{ height: 3, borderRadius: 99, backgroundColor: 'rgba(48,209,88,0.1)', overflow: 'hidden', marginBottom: '0.25rem' }}>
              <div style={{
                height: '100%', width: `${stepsPct}%`, borderRadius: 99,
            ...
            <span style={{ fontSize: '0.5rem', color: 'rgba(255,255,255,0.2)', fontWeight: 600, letterSpacing: '0.04em' }}>
              / {targetSteps.toLocaleString()} goal
            </span>
          </div>
        </div>
    ...
    <div style={{ padding: '1rem 1rem 0.75rem' }}>
      ...
      <div style={{ height: 160 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} barGap={2} margin={{ top: 0, right: 8, left: -20, bottom: 0 }}>
            <XAxis
              dataKey="day"
              axisLine={false}
              tickLine={false}
...
export const RecoveryCard: React.FC<RecoveryCardProps> = ({
  ...
  <div style={{
    ...
    border: `1px solid ${score ? recColor + '35' : 'var(--border-default)'}`,
    borderRadius: 'var(--radius-md)',
    overflow: 'hidden',
    position: 'relative',
  }}>
    ...
    <div style={{ padding: '1rem 1.125rem' }}>
      <div className="flex-row align-center justify-between" style={{ marginBottom: recLabel ? '0.625rem' : 0 }}>
        <div className="flex-row align-center gap-3" style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            width: 36, height: 36, borderRadius: 10,
            backgroundColor: score ? recColor + '18' : 'rgba(255,255,255,0.05)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            flexShrink: 0,
            boxShadow: score ? `0 0 14px ${recColor}22` : 'none',
          }}>
      ...
      <button
        ...
        style={{
          width: '100%', padding: '0.65rem 1rem',
          borderRadius: 'var(--radius-sm)',
          border: `1px solid ${score && score >= 50 ? recColor + '40' : 'rgba(255,255,255,0.1)'}`,
          background: score && score >= 50
            ? `linear-gradient(135deg, ${recColor}18, ${recColor}08)`
            : 'rgba(255,255,255,0.04)',
          color: score && score >= 50 ? recColor : 'rgba(255,255,255,0.45)',
  ...
  >
    ...
    <div className="flex-row justify-between align-center" style={{ marginBottom: '0.5rem' }}>
      <div style={{
        ...
        {/* TrendingUp icon rendered inline to avoid importing in this file */}
